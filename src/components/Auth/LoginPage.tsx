...
export const LoginPage: React.FC = () => {
  ...
  const [savedCreds] = useState<{ email: string; token: string } | null>(() => {
    const saved = localStorage.getItem('acehire_saved_credentials');
    if (saved) {
      try {
  ...
  // Form State (Default to empty if no saved credentials exist)
  const [email, setEmail] = useState(() => savedCreds?.email || '');
  const [password, setPassword] = useState(() => {
    if (savedCreds?.token) {
      try {
        return atob(savedCreds.token);
      } catch (e) {
        return '';
      }
    }
  ...
  const triggerLogin = (targetEmail: string, targetPass: string) => {
    ...
    setPendingLogin({ email: targetEmail, pass: targetPass });
  ...
  const handleConfirmSave = () => {
    if (pendingLogin) {
      // Securely store tokenized credential preference
      const encryptedToken = btoa(pendingLogin.pass || 'default123');
      localStorage.setItem(
        'acehire_saved_credentials',
        JSON.stringify({ email: pendingLogin.email, token: encryptedToken })
      );
      login(pendingLogin.email, pendingLogin.pass);
    }
  ...
  const handleDeclineSave = () => {
    if (pendingLogin) {
      localStorage.removeItem('acehire_saved_credentials');
      login(pendingLogin.email, pendingLogin.pass);
    }
    setShowSaveModal(false);
  };
  ...
  return (
    <div className="h-[calc(100vh-65px)] w-full flex flex-col items-center justify-between px-4 sm:px-6 py-4 select-none overflow-hidden relative font-sans bg-gradient-to-b from-[#060913] via-[#0B1124] to-[#120B2E] text-white">
      ...
      {/* Centered Save Login Information Popup Modal */}
      {showSaveModal && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-[#090F26] border border-blue-500/30 rounded-3xl p-6 sm:p-8 max-w-md w-full text-center space-y-6 shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
            ...
            <div className="p-4 rounded-3xl bg-blue-500/10 text-blue-400 w-16 h-16 mx-auto flex items-center justify-center border border-blue-500/20 shadow-inner">
              <Key className="w-8 h-8 text-[#38BDF8] animate-pulse" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs sm:text-sm text-slate-300 font-medium leading-relaxed">
                Save your login credentials securely for convenient and instant sign-in next time?
              </p>
            </div>
            ...
            <div className="flex items-center justify-center gap-3 pt-2">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#0284C7] to-[#2563EB] hover:from-[#0369A1] hover:to-[#1D4ED8] text-white font-extrabold text-xs shadow-lg transition-all flex items-center gap-1.5 cursor-pointer border border-[#38BDF8]/40"
              >
      ...
      {/* 1. Soft Ambient AI Lighting & Glows (Center glow reduced by 15% for soft, premium look) */}
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-35" viewBox="0 0 1440 900" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <path d="M-100 500 C 200 420, 400 580, 700 480 C 1000 380, 1200 520, 1500 450" stroke="url(#wavePurpleGrad)" strokeWidth="2.5" fill="none" />
        <path d="M-100 450 C 300 550, 600 400, 900 520 C 1200 640, 1400 460, 1600 500" stroke="url(#waveBlueGrad)" strokeWidth="2.5" fill="none" />
      </svg>

      {/* Spacer for vertical balance */}
      <div />

      {/* 2. Main Login Glass Card (Mandatory: Glow Reduced by 15%, Layout & Proportions Preserved) */}
      <div className="w-full max-w-4xl relative z-10 bg-[#070B1D]/90 backdrop-blur-2xl rounded-3xl p-6 sm:p-8 border border-slate-700/60 shadow-[0_0_35px_rgba(56,189,248,0.15)] grid grid-cols-1 md:grid-cols-12 gap-6 items-center animate-in fade-in zoom-in-95 duration-500">
        ...
        {/* Left Side: Brand Hero with Exact Welcome Page Logo */}
        <div className="md:col-span-5 flex flex-col justify-center space-y-4 pr-0 md:pr-5 md:border-r border-slate-800/80 pointer-events-none select-none">
          ...
          {/* Mandatory Change 1: Exact Identical Logo from Welcome Page */}
          ...
          <div className="space-y-1.5 pt-1">
            ...
            {/* Mandatory Change 3: Left Description Text Replaced Completely */}
            <p className="text-xs text-slate-300 font-normal leading-relaxed">
              Master interviews, coding, aptitude, and resume preparation — all in one AI platform.
            </p>
          </div>
        ...
        {/* Right Side: Authentication Form with Mandatory Hover Animations */}
        <div className="md:col-span-7 flex flex-col justify-center space-y-4">
          
          {/* Mandatory Change 5: Continue with Google Button (Hover Glow + 2-3px Scale + 0.3s Transition) */}
          <button
            onClick={() => triggerLogin('google.student@college.edu', 'googlepass')}
            type="button"
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {/* Mandatory Change 5: Sign In Button (Stronger Glow + Smooth 2-3px Scale + 0.3s Transition) */}
            <button
              type="submit"
              className="w-full py-2.5 rounded-xl bg-gradient-to-r from-[#0284C7] via-[#2563EB] to-[#9333EA] hover:from-[#0369A1] hover:via-[#1D4ED8] hover:to-[#7C3AED] text-white font-extrabold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-[0_0_35px_rgba(37,99,235,0.6)] hover:shadow-[0_0_45px_rgba(56,189,248,0.85)] hover:scale-[1.02] active:scale-95 transition-all duration-300 cursor-pointer border border-[#38BDF8]/40 mt-1 group relative overflow-hidden"
            >
              <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/30 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700 pointer-events-none" />
      ...
      {/* Mandatory Change 4: Footer Style (11px, 65% Opacity) */}
