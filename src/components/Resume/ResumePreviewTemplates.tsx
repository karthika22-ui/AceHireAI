...
import { Mail, Phone, MapPin, Linkedin, Github, Globe, Award, Briefcase, GraduationCap, Code, ExternalLink } from 'lucide-react';
...
export const ResumePreviewTemplates: React.FC<ResumePreviewTemplatesProps> = ({ data, template }) => {
  const {
    fullName,
    email,
    phone,
  ...
  // --- TEMPLATE 1: CLASSIC ATS (Traditional, single column, max ATS parseable) ---
  if (template === 'classic') {
    return (
      <div className="bg-white text-slate-900 font-serif p-8 sm:p-12 shadow-2xl rounded-sm max-w-3xl mx-auto border border-slate-200 print:shadow-none print:border-none print:max-w-none print:w-full print:p-0">
        ...
        <div className="border-b-2 border-slate-900 pb-4 text-center space-y-1">
          ...
          <div className="flex flex-wrap justify-center items-center gap-x-4 gap-y-1 text-xs text-slate-700 font-sans">
            ...
            <div className="text-xs space-y-1">
              {programmingLanguages.length > 0 && (
                <div>
                  ...
                  <span className="text-slate-800">{programmingLanguages.join(', ')}</span>
                </div>
              )}
              {technicalSkills.length > 0 && (
                <div>
                  <span className="font-bold text-slate-900">Technical Skills: </span>
                  <span className="text-slate-800">{technicalSkills.join(', ')}</span>
                </div>
              )}
              {toolsAndTech.length > 0 && (
                <div>
                  <span className="font-bold text-slate-900">Tools & Technologies: </span>
                  <span className="text-slate-800">{toolsAndTech.join(', ')}</span>
                </div>
              )}
              {programmingLanguages.length === 0 && technicalSkills.length === 0 && toolsAndTech.length === 0 && (
                <div>
                  <span className="text-slate-800">{allSkillsList.join(', ')}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {(certifications.length > 0 || achievements.length > 0 || workshops.length > 0) && (
          <div className="mt-5 space-y-2 font-sans">
            ...
            <ul className="list-disc list-inside text-xs text-slate-800 space-y-1">
              ...
              {workshops.map((w, idx) => (
                <li key={`wk-${idx}`}><span className="font-semibold">Workshop: </span>{w}</li>
              ))}
            </ul>
          </div>
        )}
  ...
  // --- TEMPLATE 2: MODERN PROFESSIONAL (Indigo accent headers, modern fonts, structured) ---
  if (template === 'modern') {
    return (
      <div className="bg-white text-slate-900 font-sans p-8 sm:p-12 shadow-2xl rounded-sm max-w-3xl mx-auto border-t-8 border-indigo-600 print:shadow-none print:border-t-8 print:max-w-none print:w-full print:p-0">
        ...
        <div className="flex justify-between items-start pb-6 border-b border-slate-200">
          <div className="space-y-1">
            ...
            <p className="text-sm font-semibold text-indigo-600">
              {education[0]?.degree || 'Aspiring Software Engineer & Placement Candidate'}
            </p>
          </div>
        ...
        {(linkedIn || gitHub || portfolio) && (
          <div className="py-2.5 px-4 bg-slate-50 border-b border-slate-200 flex flex-wrap gap-4 text-xs font-semibold text-indigo-700">
              ...
              {education.map((edu, idx) => (
                <div key={idx} className="flex justify-between items-start text-xs border-b border-slate-100 pb-2">
                  ...
                  <div className="text-right shrink-0 font-bold text-indigo-600">
                    {edu.graduationYear} {edu.cgpa && <span className="text-slate-700 font-medium">| {edu.cgpa}</span>}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="pl-4 space-y-1.5 text-xs">
              {programmingLanguages.length > 0 && (
                <div className="flex gap-2">
                  <span className="font-bold text-slate-900 min-w-[120px]">Languages:</span>
              ...
              {technicalSkills.length > 0 && (
                <div className="flex gap-2">
                  <span className="font-bold text-slate-900 min-w-[120px]">Tech Stack:</span>
                  <div className="flex flex-wrap gap-1">
              ...
              {toolsAndTech.length > 0 && (
                <div className="flex gap-2">
                  <span className="font-bold text-slate-900 min-w-[120px]">Tools:</span>
                  <div className="flex flex-wrap gap-1">
  ...
  // --- TEMPLATE 3: MINIMAL FRESHER (Clean dividers, skill emphasis, compact) ---
  return (
    <div className="bg-white text-slate-900 font-sans p-8 sm:p-12 shadow-2xl rounded-sm max-w-3xl mx-auto border border-slate-200 print:shadow-none print:border-none print:max-w-none print:w-full print:p-0">
      {/* Header */}
      <div className="text-left space-y-2 pb-5 border-b-2 border-slate-900">
      ...
      {/* Summary */}
      {summary && (
        <div className="py-4 border-b border-slate-200 space-y-1">
          ...
          <div className="space-y-2">
            ...
            {education.map((edu, idx) => (
              <div key={idx} className="text-xs space-y-0.5">
                ...
                <div className="text-slate-500 text-[10px] font-semibold">Year: {edu.graduationYear} | CGPA: {edu.cgpa}</div>
              </div>
            ))}
          </div>
        )}
      ...
      {projects.length > 0 && (
        <div className="py-5 border-b border-slate-200 space-y-3">
          <h2 className="text-xs font-black uppercase tracking-wider text-slate-900">Academic & Independent Projects</h2>
          {projects.map((proj, idx) => (
